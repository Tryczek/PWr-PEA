...
vector<int> two_opt_swap(const vector<int>& path, vector<vector<int>>& tabu_list, const vector<vector<int>>& matrix, int tabu_size) {
...
void solve(vector<vector<int>>& matrix, int iterations, int max_time_in_seconds, int tabu_size, const string& file_name) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (elapsed_time > max_time_in_seconds) {
            break;
        }

        // Zmiana elementów w œcie¿ce
        current_path = two_opt_swap(current_path, tabu_list, matrix, tabu_size);
